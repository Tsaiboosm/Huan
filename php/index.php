...
if( !empty($_GET['email']) ) $userPassword = $_GET['email'];
...
if(!empty($_GET['userName']) && !empty($_GET['userPassword']) && !empty($_GET['email']) && !empty($_GET['registered'])){
//	注册
registered($conn);
}
// 修改用户名
...
function registered($conn){
	...
	$sql = "INSERT INTO user (userName, password, email,userId)
	VALUES ('{$_GET['userName']}', '{$_GET['userPassword']}', '{$_GET['email']}','{$_GET['uuid']}')"; 
